...
BETS_COLS = [
    "record_id", "timestamp", "sport", "week", "home_team", "away_team",
    "matchup", "game_time", "bet_type", "selection", "opponent",
    "edge_pct", "stake", "predicted_margin", "point_spread",
    "weather", "status"
]
...
bet_type_filter = st.sidebar.multiselect(
    "Bet types", ["Moneyline", "Spread", "Totals", "All"], default=["All"]
)
...
def build_recs(game, sport_name):
    try:
        ...
        if "spreads" in markets:
            edges["Spread Home"] = margin
            edges["Spread Away"] = -margin
